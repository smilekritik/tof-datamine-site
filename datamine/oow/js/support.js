// OOW compatibility runtime. The executable controller is a normal ES module;
// this file only owns template compilation, lifecycle bridging, and editor hooks.
"use strict";
(() => {
  ...
  function boot(runtime, doc = document) {
    ...
    if (!registeredRootController) {
      throw new Error("dc-runtime: no static root controller was registered");
    }
    runtime.registerLogic(rootName, registeredRootController);
  ...
  let registeredRootController = null;
  const compatibilityRoot = window.TofDatamine || (window.TofDatamine = {});
  compatibilityRoot.dcRuntime = {
    getDependencies() {
      return { React: getReact(), DCLogic: StreamableLogic };
    },
    registerRootController(Controller) {
      if (typeof Controller !== "function") {
        throw new TypeError("dc-runtime: root controller must be a class or function");
      }
      registeredRootController = Controller;
    }
  };
  // src/component.ts
  ...
  function createComponentFactory(registry, ensureFetched) {
    ...
    class StreamableComponent extends React.Component {
      constructor(props) {
         ...
         *  re-render. A registry bump (new class or template) changes `ver`
         *  and breaks the memo so an
       ...
       *  (its own template or a child DC without its
  ...
  function createRuntime(doc = document) {
    ...
    function registerLogic(name, Logic) {
      const r = registry.get(name);
      r.logicError = null;
      r.Logic = Logic;
      registry.bump(name);
