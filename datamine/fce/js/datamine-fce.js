(function () {
  ...
  const UI_TEXT = {
    ui: {
      en: {
        ...
        bossLoadError: "Could not load this boss.",
        retry: "Retry"
      },
      ru: {
        ...
        bossLoadError: "Не удалось загрузить этого босса.",
        retry: "Повторить"
      }
    }
  };
    ...
    if (!boss) {
      ...
      try {
        ...
        if (state.selectedSlug === slug) {
          renderBossLoadError(shell, slug);
        }
        return;
      }
  ...
  function renderBossLoadError(shell, slug) {
    shell.innerHTML = `
      <div class="fce-card-loader fce-card-loader--error">
        <span>${escapeHtml(getUiText("bossLoadError"))}</span>
        <button class="fce-card-retry" type="button">${escapeHtml(getUiText("retry"))}</button>
      </div>
    `;
    shell.querySelector(".fce-card-retry")?.addEventListener("click", () => {
  ...
  function updateCardArt(shell, boss, displayName) {
    ...
    const showFullArtwork = () => {
      artElement.onerror = () => {
        if (artElement.getAttribute("data-fce-art-token") !== artToken) return;
        artElement.onerror = null;
        artElement.removeAttribute("src");
        artElement.classList.remove("is-preview", "is-full");
        artElement.classList.add("is-unavailable");
      };
      artElement.src = fullPath;
      artElement.classList.remove("is-preview", "is-unavailable");
    ...
    ensureImageLoaded(fullPath).then(() => {
      ...
      currentArt.classList.remove("is-preview", "is-unavailable");
