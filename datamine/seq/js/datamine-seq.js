(function () {
  ...
  const UI_TEXT = {
    en: {
      pageTitle: "TOF Sequential Scaling Charts",
    ...
    ru: {
      ...
      title: "Рост характеристик боссов по данным Datamine",
      sourceTitle: "Только данные Datamine",
  ...
  function wrapChartSvg(inner, width, height, title) {
    return `
      <svg class="seq-chart-svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}" preserveAspectRatio="xMidYMid meet" style="display:block;width:100%;height:auto;aspect-ratio:${width}/${height};min-height:280px;" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${escapeHtml(title)}">${inner}</svg>
    `.trim();
  }
  ...
  function buildAdaptiveLabelPlacements(values, pointYs, xAt, options) {
    ...
    values.forEach((item, index) => {
      ...
      let bestRect = buildLabelRect(bestPlacement.x, bestPlacement.y, textWidth, fontSize);
      let minPenalty = Infinity;

      candidates.forEach((candidate, cIndex) => {
        const rect = buildLabelRect(candidate.x, candidate.y, textWidth, fontSize);
        let overlapCount = 0;
        ...
        const outOfBounds =
          rect.left < axisLeft ||
          rect.right > axisRight ||
          rect.top < axisTop ||
          rect.bottom > axisBottom;

        const penalty = overlapCount * 100 + (outOfBounds ? 500 : 0) + cIndex * 2;
      ...
      // All points have visible labels
      placements[index] = { ...(bestPlacement || { x: pointX, y: pointY - 16 }), hidden: false };
      occupiedRects.push(
